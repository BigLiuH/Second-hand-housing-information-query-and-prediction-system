...
data=pd.read_csv("房源.csv",encoding="utf-8")
...
# search_button = tk.Button(top_frame, text="搜索", bg="#0078d7", fg="white", font=("Arial", 12))
...
def filter_data():
    ...
    if selected_years:
        ...
        years_mapping = {
            "5年以内": lambda x: float(x) >= 2019 if x != "暂无数据" else False,
            "5-10年": lambda x: 2014 <= float(x) < 2019 if x != "暂无数据" else False,
            "10年以上": lambda x: float(x) < 2014 if x != "暂无数据" else False,
        }
        ...
        for condition in selected_years_conditions:
            # 应用条件筛选年份数据
            filtered_data = origin_data[origin_data['年份'].apply(condition)]
