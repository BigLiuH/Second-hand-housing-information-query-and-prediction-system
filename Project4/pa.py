...
# 设置 CSV 文件头部
header = ['房屋信息链接','小区名称','房屋区域', '户型', '房屋面积', '朝向', '装修', '楼层', '年份', '房屋类型', '价格', '县区']
...
citys = {"沈阳":"sy"}
# qu={"铁西":"tiexi","和平":"heping1","沈河":"shenhe","浑南":"hunnan","大东":"dadong","皇姑":"huanggu","于洪":"yuhong","苏家屯":"sujiatun","沈北新区":"shenbeixinqu"}
qu={"于洪":"yuhong"}
for city, s in qu.items():
    for i in range(1, 101):
        baseurl: str = (
            f"https://sy.lianjia.com/ershoufang/{s}/pg{"" if i == 1 else i}/"
        )
        ...
        for room in rooms:
            ...
            with open(csv_file, mode='a', newline='', encoding='utf-8') as file:
                writer = csv.writer(file)
                writer.writerow(
                    [link.strip(), fields[0].strip(),fields[1].strip(), style.strip(), area.strip(), towards.strip(), decoration.strip(), floor.strip(), years.strip(), building_type.strip(), price.strip(), city.strip()])
